...
// Get reminder data (only if created by current user)
$stmt = $pdo->prepare("
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Get the user's class automatically (same as the original reminder class)
    ...
    if (empty($error)) {
        try {
            // Update reminder in database (always assigned to user's class)
            $stmt = $pdo->prepare("
                UPDATE reminders
            ...
            if ($stmt->execute([$kelasId, $categoryId, $deskripsi, $tanggal, $waktu, $reminderId, $_SESSION['user_id']])) {
                logActivity($_SESSION['user_id'], 'update_reminder', "Updated reminder: " . substr($deskripsi, 0, 50) . "...");
            ...
            $error = 'Gagal memperbarui reminder: ' . $e->getMessage();
        }
    }
}
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <input type="time" name="waktu"
                           value="<?php echo isset($_POST['waktu']) ? htmlspecialchars($_POST['waktu']) : ($reminder['waktu'] ?: ''); ?>"
                           class="w-full px-4 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none">
                </div>
            </div>
